...
//   } 
    ...
    uint32_t color32 = (color << 8 | color >> 8) << 16 | (color << 8 | color >> 8);
    uint32_t i = 0;
    uint32_t rem = len & 30;  // 计算剩余像素
    len = len - rem;            // 更新 len 为可处理的完整像素块

    // 处理部分缓冲区像素
    if (rem) {
        while (*_spi_cmd & SPI_USR);  // 等待 SPI 总线空闲
        for (i = 0; i < rem; i += 2) *spi_w++ = color32;  // 填充剩余像素
        *_spi_mosi_dlen = (rem << 4) - 1;  // 设置数据长度
#if CONFIG_IDF_TARGET_ESP32S3
        *_spi_cmd = SPI_UPDATE;  // 更新命令
        while (*_spi_cmd & SPI_UPDATE);  // 等待更新完成
#endif
        *_spi_cmd = SPI_USR;  // 发送数据
        if (!len) return;  // 如果没有剩余数据，直接返回
        i = i >> 1;  // 更新 i 为已发送的像素数
        while (i++ < 16) *spi_w++ = color32;  // 填充缓冲区
    }

    // 确保 SPI 总线空闲
    while (*_spi_cmd & SPI_USR);
    if (!rem) while (i++ < 16) *spi_w++ = color32;  // 如果没有剩余像素，填充缓冲区
    *_spi_mosi_dlen = 511;  // 设置数据长度为 511
